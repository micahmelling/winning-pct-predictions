"""
Functions to explain machine learning models, primarily SHAP and PDP.
"""
...
from helpers.helpers import make_directories_if_not_exists, transform_data_with_pipeline


def produce_shap_values_and_plots(
    model: Union[RegressorMixin, ClassifierMixin], x_df: pd.DataFrame, model_uid: str
) -> None:
    """
...
def _plot_partial_dependence(
    feature: str,
    model: Union[RegressorMixin, ClassifierMixin, Pipeline],
    x_df: pd.DataFrame,
    model_uid: str,
) -> None:
    """
    ...
    """
    save_path = os.path.join(
        "modeling", "model_results", model_uid, "diagnostics", "pdp"
    )
    make_directories_if_not_exists([save_path])
...
def produce_partial_dependence_plots(
    model: Union[RegressorMixin, ClassifierMixin, Pipeline],
    x_df: pd.DataFrame,
    model_uid: str,
) -> None:
    ...
    model.fitted_ = True
    pdp_plot_fn = partial(
        _plot_partial_dependence, model=model, x_df=x_df, model_uid=model_uid
    )
    with mp.Pool(processes=mp.cpu_count()) as pool:
        pool.map(pdp_plot_fn, list(x_df))


def run_omnibus_model_explanation_from_pipeline(
    pipeline: Pipeline, x_df: pd.DataFrame, model_uid: str
) -> None:
    """
